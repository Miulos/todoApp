  ...
  <div class="todo" :class="{ todo_selected: selected }">
    ...
    <div class="todo_body">
      ...
      <div class="todo_tasks">
        ...
        <ul>
          <li v-for="task in todayTasks" :key="task.id"><task :task="task"/></li>
        </ul>
        <h4 class="todo_subtitle" v-if="tomorrowTasks.length">Tomorrow</h4>
        <ul>
          <li v-for="task in tomorrowTasks" :key="task.id"><task :task="task"/></li>
        </ul>
        <h4 class="todo_subtitle" v-if="outdatedTasks.length">Outdated</h4>
        <ul>
          <li v-for="task in outdatedTasks" :key="task.id"><task :task="task"/></li>
        </ul>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    color () {
      return this.todo.colors[0]    // 时间08:14
    },
    progress () {
